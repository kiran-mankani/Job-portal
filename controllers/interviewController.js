...
const scheduleInterview = async (req, res) => {
  try {
    ...
    const interviewDate = date || scheduledAt;
    ...
    if (!applicationId || !interviewDate || !mode) {
      return res.status(400).json({
        success: false,
        message: "applicationId, date and mode are required",
        received: {
          applicationId: Boolean(applicationId),
          interviewDate: Boolean(interviewDate),
          mode: Boolean(mode),
        },
      });
    }
    ...
    if (normalizedMode === "online" && !trimmedMeetingLink) {
      return res.status(400).json({
        success: false,
        message:
    ...
    if (normalizedMode === "offline" && !trimmedLocation) {
      return res.status(400).json({
        success: false,
        message:
    ...
    const applicationCandidateId = application.candidate?.toString();
    ...
    if (
      candidateId &&
      applicationCandidateId !== String(candidateId)
    ) {
      return res.status(400).json({
        success: false,
        message:
    ...
    const finalCandidateId = applicationCandidateId;

    // ------------------------------------------
    // Check candidate
    // ------------------------------------------

    const candidate = await User.findById(finalCandidateId);
    ...
    if (
      String(candidate.role).trim().toLowerCase() !== "candidate"
    ) {
      return res.status(400).json({
        success: false,
    ...
    // Blocked / inactive — User schema uses isBlocked / isActive
    if (candidate.isBlocked) {
      return res.status(400).json({
        success: false,
        message: "Candidate account is blocked",
      });
    }
    ...
    if (
      !job.recruiter ||
      job.recruiter.toString() !== recruiterId
    ) {
      return res.status(403).json({
        success: false,
        message:
    ...
    let applicationWasShortlisted = false;
    ...
    if (
      ...
      applicationWasShortlisted = true;
    }

    // ------------------------------------------
    // Candidate notifications (safe)
    // ------------------------------------------
    ...
    try {
      await createNotification({
        ...
        message: `Your interview for "${job.title}" has been scheduled for ${parsedDate.toLocaleString()}.`,
        relatedId: interview._id,
        relatedType: "Interview",
      });

      if (applicationWasShortlisted) {
        await createNotification({
          userId: finalCandidateId,
          type: "application_status",
          title: "Application Shortlisted",
          message: `Your application for "${job.title}" has been shortlisted because an interview has been scheduled.`,
